...
namespace WorkFlowEngineInterface
{   
    ...
    class Program
    {
        static void main(string[] args)
        {
            ...
            workFlow.AddActivity(new WebSvcVideoEncode());
            workFlow.AddActivity(new EmailNotifyUser());
            workFlow.AddActivity(new VideoStatusChangedToProcessing());
    ...
    public class WorkFlowEngine
    {
        ...
        {
            if (workFlow == null)
                throw new InvalidOperationException("WorkFlow Doesn't have Activities to be executed.");

            foreach (var item in workFlow.WorkFlowActivities)
            {
                item.Execute();
            }
        }
    ...
    {
        public readonly List<IActivity> WorkFlowActivities = new List<IActivity>();
    ...
    public class WebSvcVideoEncode : IActivity
    {
        public void Execute()
        {
     ...
     public class VideoUpload : IActivity
    {
        public void Execute()
        {
            Console.WriteLine("Uploading Video to Cloud...Uploading Done.");
            
        }
    }

    public class VideoStatusChangedToProcessing : IActivity
    {
        public void Execute()
        {
            Console.WriteLine("Video Record Status: Change to Processing.");
        }

    }

    public class EmailNotifyUser : IActivity
    {
        public void Execute()
        {
            Console.WriteLine("Sending Email to Owner. Video started processing...");
        }
